...
PROCESS_URL = "http://localhost:8000/process_urls/"
...
def main():
    ...
    with col1:
        ...
        input_method = st.radio("Choose input method:", ("Upload Files", "Choose from Online List"))
        ...
        if input_method == "Upload Files":
            ...
            if not df.empty:
                ...
                for i, row in df.iterrows():
                    if any(pd.isna(row[col]) for col in ['IMAGE_LINK', 'TITLE', 'SUMMARY', 'PDF_LINK']):
                        continue
                    ...
                    with col2:
                        ...
                        if st.button(f"Select {row['TITLE']}", key=f"select_{i}"):
                            with st.spinner(f"Processing {row['TITLE']}..."):
                                response = requests.post(PROCESS_URL, json={
                                    "file_name": row['TITLE'],
                                    "s3_pdf_link": row['PDF_LINK']
                                })
                                if response.status_code == 200:
                                    st.success(f"{row['TITLE']} processed and added to index!")
                                else:
                                    st.error(f"Error processing {row['TITLE']}.")
