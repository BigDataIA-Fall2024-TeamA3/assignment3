...
from utils.s3_utils import list_buckets, list_s3_documents, upload_file, list_s3_documents, download_file, check_connection
from config import fastapi_config # Contains env variables, access by eg: "fastapi_config.AWS_ACCESS_KEY_ID"
from routers import rag
from utils.document_processors import load_multimodal_data, load_data_from_directory, load_multimodal_data_from_s3_file_urls
...
async def process_urls(data: UrlData):
    try:
        initialize_settings()
        documents = load_multimodal_data_from_s3_file_urls(data.file_name, data.s3_pdf_link)
        print("process_urls: done")
        index = create_index(documents)
        print("process_urls: done")
        return JSONResponse(content={"message": "Files processed and index created!"})
    except Exception as e:
